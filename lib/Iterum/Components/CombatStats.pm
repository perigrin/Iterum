...
sub add ($ecs, $entity_id, %options) {
    ...
    $ecs->add_component($entity_id, 'CombatStats', {
        attack        => $attack,
        defense       => $defense,
...
sub add_attack_buff ($ecs, $entity_id, $buff) {
    ...
    croak "Buff value must be numeric" unless $buff->{value} =~ /^-?\d+(?:\.\d+)?$/;
    croak "Buff must have a duration" unless defined $buff->{duration};
    croak "Buff duration must be a positive integer" unless $buff->{duration} =~ /^\d+$/ && $buff->{duration} > 0;
