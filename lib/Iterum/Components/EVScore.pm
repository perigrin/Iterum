...
sub add ($ecs, $entity_id, %options) {
    ...
    # Validation
    croak "decisions must be an array reference" unless ref $decisions eq 'ARRAY';
    
    $ecs->add_component($entity_id, 'EVScore', {
        decisions        => $decisions,
...
sub record_decision ($ecs, $entity_id, $decision) {
    ...
    croak "Decision must include an ev_score" unless defined $decision->{ev_score};
...
sub get_stats ($ecs, $entity_id) {
    ...
    foreach my $action (keys %{$stats->{action_counts}}) {
        $stats->{action_avgs}{$action} = $action_scores{$action} / $stats->{action_counts}{$action};
    }
