...
class Iterum::Systems::Combat {
    ...
    method start_combat($entity1, $entity2) {
        ...
        foreach my $entity ($entity1, $entity2) {
            my ($health, $stats) = $ecs->get_components($entity, 'Health', 'CombatStats');
            croak "Entity does not have required components" unless $health && $stats;
        }
    ...
    method are_in_combat($entity1, $entity2) {
        return (exists $combatants{$entity1} && exists $combatants{$entity1}{$entity2});
    }
    ...
    method set_action($entity, $action) {
        croak "Invalid action: $action" unless $action =~ /^(attack|defend|none)$/;
    ...
    method calculate_attack($attacker, $defender) {
        ...
        my $attack = Iterum::Components::CombatStats::effective_attack($ecs, $attacker);
        
        # Get defender's defense value
        my $defense = Iterum::Components::CombatStats::effective_defense($ecs, $defender);
