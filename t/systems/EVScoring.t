...
subtest 'Required Components' => sub {
    ...
    is( \@required, ['EVScore'], 'Required components are correct' );
};
...
subtest 'Recording Player Decisions' => sub {
    ...
    is( $ev_score->{current_decision}->{action},
        'attack', 'Action is recorded correctly' );
    ...
    is( $ev_score->{current_decision}->{action},
        'defend', 'New action is recorded' );
    is( scalar @{ $ev_score->{decisions} },
        2, 'Decision history contains two entries' );

    # First decision should still be in history
    is( $ev_score->{decisions}[0]->{action},
        'attack', 'First decision preserved in history' );
};
