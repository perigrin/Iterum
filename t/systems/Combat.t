...
subtest 'Initiating Combat' => sub {
    ...
    # Create two entities
    my $entity1 =
      create_entity( $ecs, label => 'entity1', attack => 15, defense => 5 );
    my $entity2 =
      create_entity( $ecs, label => 'entity2', attack => 10, defense => 8 );
...
subtest 'Applying Damage' => sub {
    ...
    is( $health->{current_hp}, 0,
        'Health reduced to 0 when damage exceeds it' );
};
...
subtest 'Multiple Entities Combat' => sub {
    ...
    ok(
        $combat->are_in_combat( $entity1, $entity2 ),
        'Entity1 and Entity2 are in combat'
    );
    ok(
        $combat->are_in_combat( $entity2, $entity3 ),
        'Entity2 and Entity3 are in combat'
    );

    # Entity1 and Entity3 should not be in combat with each other
    ok(
        !$combat->are_in_combat( $entity1, $entity3 ),
        'Entity1 and Entity3 are not in combat'
    );
