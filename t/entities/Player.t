...
subtest 'Player creation' => sub {
    ...
    my ($combat_stats) = $ecs->get_components($custom_player->id, 'CombatStats');
...
subtest 'Player attack' => sub {
    ...
    is($enemy_health->{current_hp}, 50 - $damage, 'Enemy health reduced by damage amount');
...
subtest 'Player damage and healing' => sub {
    ...
    my ($health) = $ecs->get_components($player->id, 'Health');
    is($health->{current_hp}, 70, 'Player health reduced after taking damage');
    ...
    ($health) = $ecs->get_components($player->id, 'Health');
    is($health->{current_hp}, 100, 'Player health capped at max_hp after healing');
    
    # Test death
    $player->take_damage(200); # More than current health
    ($health) = $ecs->get_components($player->id, 'Health');
    is($health->{current_hp}, 0, 'Player health reduced to 0 on lethal damage');
    ok(!Iterum::Components::Health::is_alive($ecs, $player->id), 'Player is not alive after lethal damage');
};
