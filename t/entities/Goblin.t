...
subtest 'Goblin creation' => sub {
    ...
    my ($combat_stats) = $ecs->get_components($custom_goblin->id, 'CombatStats');
...
subtest 'Goblin attack' => sub {
    ...
    is($player_health->{current_hp}, 100 - $damage, 'Player health reduced by damage amount');
};
...
subtest 'Goblin AI decision making' => sub {
    ...
    is($decision->{action}, 'none', 'Default action is none when no player_id is provided');
    ...
    is($decision->{action}, 'attack', 'Goblin chooses to attack when adjacent');
    ...
    $decision = $goblin->make_decision($player->id);
    is($decision->{action}, 'move', 'Goblin chooses to move when not adjacent');
    ...
    $decision = $goblin->make_decision($player->id);
    is($decision->{action}, 'attack', 'Goblin still attacks when low on health (reckless)');
};
...
subtest 'Goblin Combat system integration' => sub {
    ...
    # Start combat
    $combat_system->start_combat($goblin->id, $player->id);
    ok($combat_system->are_in_combat($goblin->id, $player->id), 'Goblin and player are in combat');
    ...
    ok($combat_system->last_hit($goblin->id) || !$combat_system->last_hit($goblin->id), 
       'Combat system processed goblin attack');
       
    # End combat
    $combat_system->end_combat($goblin->id, $player->id);
    ok(!$combat_system->are_in_combat($goblin->id, $player->id), 'Combat ended successfully');
};
