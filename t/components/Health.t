...
subtest 'Custom Health Values' => sub {
    ...
    my $entity_id = $ecs->new_entity('test_entity');
    Iterum::Components::Health::add($ecs, $entity_id, max_hp => 200, current_hp => 150);
...
subtest 'Health Validation' => sub {
    ...
    like(
        dies { Iterum::Components::Health::add($ecs, $entity_id, max_hp => -50) },
        qr/max_hp must be positive/,
        'Negative max_hp throws error'
    );
    
    like(
        dies { Iterum::Components::Health::add($ecs, $entity_id, current_hp => -10) },
    ...
    like(
        dies { Iterum::Components::Health::add($ecs, $entity_id, max_hp => 50, current_hp => 100) },
...
subtest 'Health Modification' => sub {
    ...
    my $entity_id = $ecs->new_entity('test_entity');
    Iterum::Components::Health::add($ecs, $entity_id, max_hp => 100, current_hp => 100);
    ...
    ($health) = $ecs->get_components($entity_id, 'Health');
    is($health->{current_hp}, 100, 'Healing does not increase HP above max_hp');
};
...
subtest 'Health Status Checks' => sub {
    ...
    ok(Iterum::Components::Health::is_alive($ecs, $entity_id), 'Entity is alive at full health');
    ok(Iterum::Components::Health::is_full_health($ecs, $entity_id), 'Entity is at full health');
    
    Iterum::Components::Health::damage($ecs, $entity_id, 50);
    ok(Iterum::Components::Health::is_alive($ecs, $entity_id), 'Entity is still alive at half health');
    ok(!Iterum::Components::Health::is_full_health($ecs, $entity_id), 'Entity is not at full health after damage');
    
    Iterum::Components::Health::damage($ecs, $entity_id, 50);
    ok(!Iterum::Components::Health::is_alive($ecs, $entity_id), 'Entity is not alive at 0 health');
};
