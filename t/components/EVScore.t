...
subtest 'Default EVScore Values' => sub {
    ...
    is($ev_score->{current_decision}, undef, 'Default current decision is undef');
...
subtest 'Recording Decisions' => sub {
    ...
    my $result = Iterum::Components::EVScore::record_decision($ecs, $entity_id, $decision);
    ...
    Iterum::Components::EVScore::record_decision($ecs, $entity_id, $decision2);
    
    # Check the updated component again
    ($ev_score) = $ecs->get_components($entity_id, 'EVScore');
    is($ev_score->{current_decision}, $decision2, 'Current decision is updated');
...
subtest 'Decision Statistics' => sub {
    ...
    foreach my $decision (@decisions) {
        Iterum::Components::EVScore::record_decision($ecs, $entity_id, $decision);
    }
    ...
    is($stats->{action_avgs}{attack}, $expected_attack_avg, 'Attack average score is correct');
    is($stats->{action_avgs}{defend}, $expected_defend_avg, 'Defend average score is correct');
    
    # Overall average
    my $expected_overall_avg = (75 + 85 + 90) / 3;
    is($stats->{overall_avg}, $expected_overall_avg, 'Overall average score is correct');
};
