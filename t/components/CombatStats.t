...
subtest 'Stat Buffs' => sub {
    ...
    Iterum::Components::CombatStats::add_attack_buff(
        $ecs,
        $entity_id,
        {
            value    => 5,
    ...
    is( $stats->{attack_buffs}[0]{duration},
        3, 'Attack buff duration is correct' );

    # Test adding defense buff
    Iterum::Components::CombatStats::add_defense_buff(
        $ecs,
        $entity_id,
        {
            value    => 3,
    ...
    is( $stats->{defense_buffs}[0]{duration},
        2, 'Defense buff duration is correct' );

    # Test calculating effective stats
    is( Iterum::Components::CombatStats::effective_attack( $ecs, $entity_id ),
        15, 'Effective attack includes buffs' );
    ...
    ($stats) = $ecs->get_components( $entity_id, 'CombatStats' );
    is( $stats->{attack_buffs}[0]{duration},
        2, 'Attack buff duration decreased' );
    is( $stats->{defense_buffs}[0]{duration},
        1, 'Defense buff duration decreased' );
    ...
    is( scalar @{ $stats->{defense_buffs} }, 0,
        'Expired defense buff removed' );

    # Verify effective stats return to base after buffs expire
    is( Iterum::Components::CombatStats::effective_attack( $ecs, $entity_id ),
        10, 'Effective attack returns to base after buffs expire' );
